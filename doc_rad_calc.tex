...
From the formula:\\
A/q = $\frac{B \cdot \rho \cdot e}{\gamma\cdot\beta \cdot c \cdot u}$ \\
we can see that the determination of A/q is limited by the radius $\rho$ and time measurement (and the combined pathlength = $\rho\cdot \omega$. Here it is omitted, as the trajectory fluctuations are 2nd or 3rd order).\\
That means:\\
\newline
